...
internal class ManifestEmbeddedFileProviderEx : IFileProvider
{
    ...
    public ManifestEmbeddedFileProviderEx(Assembly assembly)
    {
        // Validate the parameters before attempting to use them.
        ArgumentNullException.ThrowIfNull(assembly, nameof(assembly));

        // Create the innder provider.
        InnerProvider = new ManifestEmbeddedFileProvider(assembly);
    }
        ...
        string root)
    {
        ...
        // Create the innder provider.
        InnerProvider = new ManifestEmbeddedFileProvider(assembly,root);
    }
        ...
        DateTimeOffset lastModified)
    {
        ...
        // Create the innder provider.
        InnerProvider = new ManifestEmbeddedFileProvider(assembly, root, lastModified);
    }
        ...
        DateTimeOffset lastModified)
    {
        ...
        ArgumentNullException.ThrowIfNull(assembly, nameof(assembly));
        ArgumentException.ThrowIfNullOrEmpty(root, nameof(root));
        ArgumentException.ThrowIfNullOrEmpty(manifestName, nameof(manifestName));

        // Create the innder provider.
        InnerProvider = new ManifestEmbeddedFileProvider(assembly, root, manifestName, lastModified);
    }
    ...
    {
        // Validate the parameters before attempting to use them.
        ArgumentException.ThrowIfNullOrEmpty(subpath, nameof(subpath));
    ...
    public IFileInfo GetFileInfo(string subpath)
    {
        ...
        if (subpath.StartsWith("/_content/"))
        {
            ...
            subpath = subpath["/_content/".Length..];
            ...
            if (subpath.StartsWith($"{asmName}"))
            {
                ...
                subpath = subpath[(asmName ?? "").Length..];
            }
        }
